...
\newcommand{\tutorialvdw}{\url{\urlstring/free_energy/methane_water}}
\newcommand{\tutorialdgsolv}{\url{\urlstring/free_energy/ethanol_water}}
\newcommand{\tutorialdgbind}{\url{\urlstring/free_energy/complex}}
...
%%% ARTICLE SETUP
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{From Proteins to Perturbed Hamiltonians: A Suite of Tutorials for the GROMACS-2025 Molecular Simulation Package [Article v\versionnumber]}
...
\begin{enumerate}
	\item Describe the content of a GROMACS system topology
...
\begin{enumerate}
	\item Interpret the contents and organization of GROMACS force field files and how parameters from different, but compatible, sources can be added to them
...
After completing Tutorial 3, "Protein-Ligand Complex," the user should be able to construct a solvated protein system including a ligand that needs to be parametrized. Specific learning objectives include:
\begin{enumerate}
...
After completing Tutorial 4, "van der Waals Transformation of Methane," the user should be able to carry out a series of simulations for the alchemical transformation of a small molecule in water. Specific learning objectives include:
\begin{enumerate}
	\item Define the roles of \texttt{.mdp} keywords related to free energy calculations
...
After completing Tutorial 5, "Ethanol Hydration Free Energy,'" the user should be able to perform a complete alchemical transformation of a small molecule in water, including both van der Waals and electrostatic terms. Specific learning objectives include:
\begin{enumerate}
	\item Define $\lambda$-spacing for Coulombic and Lennard-Jones terms
	\item Compute the value of $\Delta$G\textsubscript{hydr} for a polar molecule
\end{enumerate}

After completing Tutorial 6, "Protein-Ligand Binding Free Energy," the user should be able to...
\begin{enumerate}
	\item Construct ligand virtual sites that are used to reflect the ligand internal geometry and binding pose
	\item Employ orientation restraints in a protein-ligand complex
	\item Calculate $\Delta$G\textsubscript{bind} after a separate alchemical transformation in water
\end{enumerate}

After completing Tutorial 7, "Umbrella Sampling," the user should be able to perform simulations under the influence of a biasing potential. Specific learning objectives include:
\begin{enumerate}
	...
	\item Define "reaction coordinate" and determine how to construct a suitable one
	\item Perform restrained simulations in multiple sampling windows along a reaction coordinate
	\item Compute a potential of mean force profile, including appropriate corrections for volume entropy effects
\end{enumerate}

After completing Tutorial 8, "Biphasic Systems," the user should be able to construct heterogeneous systems of two different liquids. Specific learning objectives include:
\begin{enumerate}
...
After completing Tutorial 9, "Virtual Sites," the user should be able to understand what virtual sites are and how they are used in specific cases. Specific learning objectives include:
\begin{enumerate}
...
For visualization of coordinate files and trajectories, many options are available, including Visual Molecular Dynamics (VMD, \url{http://www.ks.uiuc.edu/Research/vmd/})~\cite{Humphrey1996}, Chimera (\url{https://www.cgl.ucsf.edu/chimera/})~\cite{Petersen2004}, and PyMOL (\url{https://pymol.org/2/}), among others. Users should be familiar with at least one of these programs for visualizing the system. The latest version of each of these programs (VMD 1.9.4, ChimeraX 1.12, and PyMOL 3.1 at the time this article was written) is preferred for maximum compatibility with different file formats and features.

The protein-ligand complex tutorial will make use of the Avogadro molecular editor (\url{https://avogadro.cc/})~\cite{Hanwell2012}. Other accessory scripts require a Python interpreter in the 3.x version series; they have generally been tested and used with Python versions 3.11 and 3.13.
...
Beyond the consideration of whether or not a force field has parameters for a given chemical species, it is important to realize that the choice of force field is perhaps the single most critical aspect to beginning a new simulation project. No existing force field perfectly treats all species, and each available force field has pros and cons for different species. It is incumbent upon the user to make an informed, justifiable choice based on literature reading and evaluation of the force field parametrization protocol to understand to which purposes a given force field is best suited. There is no catch-all answer or universally "best" force field. For this tutorial, we will be using CHARMM36m~\cite{Huang2016}, a widely used all-atom force field. Download the force field from the MacKerell lab website (\url{https://mackerell.umaryland.edu/charmm_ff.shtml#gromacs}); for this tutorial we will use the files contained in the ``charmm36-feb2026\_cgenff-5.0.ff.tgz'' tarball. Unarchive the tarball and either leave the resulting ``charmm36-feb2026\_cgenff-5.0.ff'' directory in the working directory or move it to \texttt{\$GMXLIB}. The first prompt is for the user to select the force field that will be applied to the system. 

Once a starting structure has been chosen and the force field decided upon, run \texttt{pdb2gmx} to create the topology:
...
\end{lstlisting}
%
The \texttt{nstxout} and \texttt{nstvout} settings control the interval of time steps between writing coordinates (x) and velocities (v) to the trajectory (\texttt{.trr}) file. With these settings, coordinates and velocities are saved every 5 ps. Similarly, energy terms are written to the \texttt{.edr} file (\texttt{nstenergy}) and \texttt{.log} file (\texttt{nstlog}) at the same interval.
...
After the run, analyze the temperature time series to verify that the target temperature (298 K) was achieved and maintained, using the \texttt{energy} program. At the prompt, type \texttt{16 0}, then Enter. Group 16 is the temperature time series, and 0 terminates input to \texttt{energy}.
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{plot_lyso_nvt_temperature}
...
After the run has concluded, again use the \texttt{energy} program, in this case to extract the pressure time series. Enter 17 to select Pressure, and 0 to terminate input to \texttt{energy}:

\begin{lstlisting}
$ gmx energy -f npt.edr -o pressure.xvg
\end{lstlisting}

The pressure time series is plotted in Figure~\ref{lyso_npt_pres_fig}. The screen output also indicates that the average pressure was -3 $\pm$ 11 bar. The target value was 1 bar. This discrepancy is often a source of concern for new users. Consider, however, the large error bar associated with the time series, and the wide range of pressure values sampled during the simulation (Figure~\ref{lyso_npt_pres_fig}). Pressure is a quantity that fluctuates wildly in a microscopic system. As a result, instantaneous values are going to vary considerably, and average values may not correspond exactly to the desired target value. However, given the large standard deviation of pressure during NPT equilibration (11 bar), the average obtained during the simulation (-3 bar) is statistically indistinguishable from the target of 1 bar. It is possible to extend the NPT equilibration phase for additional time in an attempt to obtain better agreement, but it is not necessary to do so. The existing agreement is sufficient.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{plot_lyso_npt_pressure}
\caption{Pressure of the solvated lysozyme system over time during NPT equilibration.}
\label{lyso_npt_pres_fig}
\end{figure}

An additional indicator of the convergence of NPT equilibration is the density of the system. Density is less prone to the fluctuations observed in the instantaneous pressure values. Again, use the \texttt{energy} program to extract the density time series from the \texttt{npt.edr} file, choosing 23 for density and 0 to terminate input:

\begin{lstlisting}
$ gmx energy -f npt.edr -o density.xvg
\end{lstlisting}

The density time series is shown in Figure~\ref{lyso_npt_dens_fig}. The average value from NPT equilibration is 1025.3 $\pm$ 0.5 kg m\textsuperscript{-3}. Note that this value is about 2.7\% percent higher than the expected value for bulk TIP3P water (986 kg m\textsuperscript{-3})~\cite{Jorgensen1983}. Such an outcome is not unusual, as the system contains a protein and ions, therefore its density will not be exactly equivalent to that of pure water.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{plot_lyso_npt_density}
...
\end{lstlisting}
%
Note that in this \texttt{grompp} command, there is no use of \texttt{-r}, as no restraints are being applied. The \texttt{.mdp} settings are the same as during NPT equilibration, except no restraints are specified and the run is being carried out for 5,000,000 steps, corresponding to 10 ns (5,000,000 steps $\times$ 0.002 ps per step = 10,000 ps = 10 ns).

As before, it is useful to name files in accordance with the time interval and process being simulated. Here, the user will perform a 10-ns, unrestrained MD simulation, starting from t = 0 ns (equilibration time is not counted) and proceeding to t = 1 ns, hence the base file name of \texttt{md\_0\_10}. To run the simulation, invoke \texttt{mdrun}:
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{plot_lyso_md_rg}
\caption{Radius of gyration of lysozyme during the production MD simulation. Solid lines correspond to 500-ps (50-frame) running averages.}
\label{lyso_md_rg_fig}
\end{figure}

The R\textsubscript{g} time series is shown in Figure~\ref{lyso_md_rg_fig}. This quantity is similarly stable over this short simulation, which is expected given the stable fold of lysozyme and the very short nature of the simulation. Large structural changes are not expected within 10 ns for any protein.

One can also look at the evolution of secondary structure in a protein. This analysis is carried out with the \texttt{dssp} program, which uses the DSSP (Dictionary of Secondary Structure of Proteins) algorithm~\cite{Kabsch1983}:

\begin{lstlisting}
$ gmx dssp -s md_0_10.tpr -f md_0_10_noPBC.xtc -tu ns -o dssp.dat -num dssp_num.xvg
\end{lstlisting}

The output of the DSSP analysis is shown in Figure~\ref{lyso_md_dssp_fig}.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{plot_lyso_md_dssp}
\caption{Evolution of secondary structure elements during the production MD simulation. Solid lines correspond to 500-ps (50-frame) running averages.}
\label{lyso_md_dssp_fig}
\end{figure}

The last analysis to be presented here regards hydrogen bonds. GROMACS uses two criteria to determine if a hydrogen bond exists between two groups: (1) the donor-H distance (defaults to 0.35 nm or less, can be tuned with \texttt{-hbr}) and (2) the donor-acceptor-H angle (defaults to 30\textdegree, can be tuned with \texttt{-hba}). Note the order of atoms in the angle criterion; GROMACS does not use the donor-H-acceptor angle as is employed in other software. Hydrogen bonds are highly directional and are strongest when linear, so the use of donor-acceptor-H $\le$ 30\textdegree means that donor-H-acceptor angles will range from 150\textdegree $-$ 180\textdegree.
...
Select the ``MainChain+H'' group (7) for both selections when prompted. In GROMACS, the ``MainChain'' group contains the N, C$\alpha$, C, and O atoms. ``MainChain+H'' includes those four atoms, plus the amide H atom, which is required for assignment of hydrogen bonds. In GROMACS convention, ``Backbone'' contains only N, C$\alpha$, and C atoms; no hydrogen bonds can be computed from this group, despite the vernacular of ``backbone hydrogen bonds.''
...
Plotted together, the results should resemble what is shown in Figure~\ref{lyso_md_hbond_fig}:

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{plot_lyso_md_hbonds}
\caption{Evolution of hydrogen bonds between various groups during the production MD simulation. Solid lines correspond to 500-ps (50-frame) running averages.}
\label{lyso_md_hbond_fig}
\end{figure}
...
%%%%%%%%% FUNDAMENTALS: PROTEIN-LIGAND COMPLEX %%%%%%%%%
\subsection{Tutorial 3: Protein-Ligand Complex} \label{prot_lig}
...
In this tutorial (available online at \tutorialcomplex), the protein topology will be generated with the CHARMM36m force field~\cite{Huang2016}, a recent revision of the CHARMM36 force field~\cite{Best2012}. The ligand topology will be generated with CGenFF via its web server (\url{https://cgenff.com}). Download the CHARMM36m force field in GROMACS format from the MacKerell lab website (\url{http://mackerell.umaryland.edu/charmm_ff.shtml#gromacs}). Extract the force field archive in the working directory or move it to \texttt{\$GMXLIB}, as in Tutorial 1 (Section~\ref{lyso}):
...
The files produced by the CGenFF server will require some modification to be used directly. The server will provide a topology called "jz4\_fix\_gmx.top," which is a GROMACS system topology, meaning it is intended for standalone use. We will need to adapt it for our purpose. First, copy it to a file called "jz4\_gmx.itp":

\begin{lstlisting}
$ cp jz4_fix_gmx.top jz4_gmx.itp
\end{lstlisting}

Make the following modifications to "jz4\_gmx.itp":

\begin{enumerate}
	\item Remove the \texttt{\#include "charmm36-feb2026\_cgenff-5.0.ff/\\forcefield.itp"} line
	\item Replace the \texttt{\#include "charmm36-feb2026\_cgenff-5.0.ff/\\jz4\_ffbonded.itp"} line with the actual content of that file
	\item Remove all lines from \texttt{; Include water topology} to the end of the file
	\item Rename the \texttt{[moleculetype]} from Other to JZ4
	\item Change \texttt{\#include "posre.itp"} to \texttt{\#include "posre\_jz4.itp"}
\end{enumerate}
...
The solvated, neutralized system will be energy-minimized and equilibrated in two phases, $NVT$ and $NPT$, following the same protocol as the lysozyme system described above (Sections~\ref{lyso_em} and~\ref{lyso_equil}).
...
Then, continue with $NVT$ and $NPT$ equilibration as in Tutorial 1 (Section~\ref{lyso}).
...
\end{lstlisting}
%
The \texttt{distance} program will produce a time series to \texttt{dist.xvg} and will print statistics to the terminal. The average distance is 0.43 $\pm$ 0.17 nm (4.3 $\pm$ 1.7 \AA), which is slightly longer than the typical threshold for a hydrogen bond. Over the last 5 ns of the simulation (which can be calculated by adding -b 5000 to the command above), the average distance is 0.30 $\pm$ 0.03 nm (3.0 $\pm$ 0.3 \AA), consistent with a hydrogen bond. Therefore, after some initial equilibration time, the ligand settles into a hydrogen-bonded pose, at least with respect to the distance between the two oxygen atoms.
...
\end{lstlisting}
%
The average value is 32 $\pm$ 15\textdegree (24 $\pm$ 7\textdegree over the last 5 ns), which is somewhat unexpected; if the angle is defined as OAB - H12 - OE1 and the hydrogen bond is maintained (as it appears to from visual inspection of the trajectory), why is it that the value is so low and not somewhat closer to 180\textdegree? Inspect the contents of \texttt{index.ndx} and group 25 will contain:
...
\end{lstlisting}
%
Using this index group as an input into the \texttt{angle} program yields an average value of 139 $\pm$ 15\textdegree (146 $\pm$ 9\textdegree over the last 5 ns).
...
\end{lstlisting}
%
Note the use of \texttt{em.tpr} as the reference structure. The RMSD is thus being computed using the energy-minimized crystal coordinates as reference. Over the 10-ns simulation, the ligand RMSD plateaus at a value of 0.15 $\pm$ 0.06 nm (1.6 $\pm$ 0.6 \AA). Such a low value indicates that the binding pose changed very little on this short time scale.
...
\end{lstlisting}
%
The output should indicate that the short-range Coulombic interaction energy is -16.5 $\pm$ 10.0 kJ mol\textsuperscript{$-$1} and the short-range Lennard-Jones interaction energy is -96.8 $\pm$ 7.4 kJ mol\textsuperscript{$-$1}. The decomposition of these terms, regardless of force field, has no physical meaning. Only the total interaction energy may be considered a useful metric, again only if the force field has been parametrized in such a way that makes this quantity useful. Thus, the total interaction energy between 2-propylphenol and T4 lysozyme over the 10-ns MD simulation performed here is -113.3 $\pm$ 12.4 kJ mol\textsuperscript{$-$1}.
...
%%%%%%%%% ADVANCED: VDW TRANSFORMATION OF METHANE %%%%%%%%%
\subsection{Tutorial 4: van der Waals Transformation of Methane} \label{vdw}
...
\begin{equation} \label{vdw_hamiltonian_eq}
...
\subsubsection{Free Energy Settings} \label{vdw_settings}
...
\subsubsection{Workflow} \label{vdw_workflow}
...
\subsubsection{Analysis} \label{vdw_ana}
...
\subsubsection{Summary and Review of Objectives} \label{vdw_summary}
...
The user was introduced to settings related to free energy in Section~\ref{vdw_settings} and analysis using BAR was performed in section~\ref{vdw_ana}. Several output files were plotted and their contents discussed. Interested readers may wish to refer to the Multistate BAR (MBAR) method~\cite{Shirts2008}, in which free energy differences are computed from all states. A Python implementation for MBAR is available from \url{https://github.com/choderalab/pymbar} and is fully compatible with GROMACS output. The user simply needs to increase the value of \texttt{calc\_lambda\_neighbors} in the \texttt{.mdp} file to evaluate free energy differences among all $\lambda$ states.


%%%%%%%%% ADVANCED: FREE ENERGY OF SOLVATION %%%%%%%%%

\subsection{Tutorial 5: Ethanol Hydration Free Energy} \label{dgsolv}

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{etoh_solv}
\caption{Ethanol in water.}
\label{etoh_solv_fig}
\end{figure}

This tutorial assumes that the user has completed Tutorial 4 (Section~\ref{vdw}), as it builds upon those concepts by performing a complete transformation of both van der Waals and electrostatic terms in a small solute. In this case, the solute is ethanol, which has an experimentally calculated value of its hydration free energy ($\Delta$G\textsubscript{hydr}). This tutorial is available online at \tutorialdgsolv.

The free energy change of transforming a system from state A to state B, $\Delta$G\textsubscript{AB}, is a function of a coupling parameter, $\lambda$, which indicates the level of change that has taken place between states A ($\lambda$ = 0) and B ($\lambda$ = 1). $\lambda$ therefore takes on values within \{0..1\} and represents the extent to which the Hamiltonian has been perturbed and the system has been transformed. The Hamiltonian at a given value of $\lambda$ is therefore a combination of the A- and B-state energy functions:

\begin{equation} \label{eqn_hamiltonian_AB}
	H(\lambda) = H^{A}(1 - \lambda) + H^{B}(\lambda)
\end{equation}

Simulations conducted at different values of $\lambda$ allow us to plot a $\partial$H/$\partial\lambda$ curve, from which $\Delta$G\textsubscript{AB} is obtained. The first step in planning free energy calculations is how many $\lambda$ points will be used to describe the transformation from state A ($\lambda$ = 0) to state B ($\lambda$ = 1), with the goal of collecting adequate data to exhaustively sample phase space and produce a reliable $\partial$H/$\partial\lambda$ curve.

For decoupling Coulombic interactions, which depend linearly upon $\lambda$, an equidistant $\lambda$ spacing from 0 to 1 should usually suffice, with $\lambda$ spacing values of of 0.05 $-$ 0.1 being common. Note that this is a broad generalization, and in fact many molecules will require a great deal more finesse, such as those that interact very strongly with the surrounding environment through hydrogen bonding. In this case, $\lambda$ spacing may need to be decreased, such that more points are used, perhaps even in an asymmetric fashion.

For decoupling van der Waals interactions, $\lambda$ spacing can be much trickier. A larger number of $\lambda$ points may be necessary to properly describe the transformation. Clustering $\lambda$ points in regions where the slope of the $\partial$H/$\partial\lambda$ curve is steep may be necessary. For the purposes of this tutorial, we will use equidistant $\lambda$ spacing of 0.05, but in many cases, one may need to use different spacing, especially clustering values in the range of 0.6 $\le \lambda \le$ 0.8.

\subsubsection{Prepare the System} \label{dgsolv_prep}

The system used for this tutorial is a single molecule of ethanol solvated in explicit water. The force field to be used is CHARMM, specifically the CHARMM General Force Field (CGenFF)~\cite{Vanommeslaeghe2010} for the ethanol solute and the CHARMM-modified TIP3P water model~\cite{Jorgensen1983,Durell1994,Neria1996} for the solvent. Download the CHARMM36 force field in GROMACS format from \url{http://mackerell.umaryland.edu/charmm_ff.shtml#gromacs}.

The coordinates for ethanol are provided in PDB format in the online version of the tutorial and with the files that accompany this article. Given that ethanol is a core model compound in CGenFF, the user can produce its topology directly using \texttt{pdb2gmx} and subsequently solvate it in a 4-nm (40-\AA) cubic box of TIP3P water with the following commands:

\begin{lstlisting}
$ gmx pdb2gmx -f etoh.pdb -o etoh.gro -p topol.top -water tip3p
$ gmx editconf -f etoh.gro -o etoh_box.gro -c -box 4.0
$ gmx solvate -cp etoh_box.gro -cs spc216.gro -o etoh_solv.gro -p topol.top
\end{lstlisting}

\subsubsection{Run the Free Energy Calculations} \label{dgsolv_run}

For these calculations, we will perform a series of 31 simulations, first transforming the Coulombic terms (such that charges on each atom are zero), then transforming the Lennard-Jones terms. It is critical that van der Waals interactions not be perturbed for atoms with partial charge. If repulsive Lennard-Jones terms are removed, charges will collapse in on themselves, resulting in a numerical singularity (infinite force) and the simulation will crash. Therefore, only once charges have been ``turned off'' in the first set of $\lambda$ states, it is safe to transform the Lennard-Jones terms. Refer to the input set of $\lambda$ vectors in the online tutorial to see the exact configuration of $\lambda$ states for each type of interaction.

As in Tutorial 4 (Section~\ref{vdw}), the free energy calculations will be performed as a series of simulations, with the output of the previous $\lambda$ state serving as the input for the current $\lambda$ state. The Bash script that performs these simulations is given below.
...
To calculate the value of $\Delta$G\textsubscript{hydr}, again invoke \texttt{gmx bar} on the resulting \texttt{*.xvg} files from each production MD simulation. The result will look something like:
...
Assuming reversibility, the result means that $\Delta$G\textsubscript{hydr} = -17.8 $\pm$ 0.2 kJ mol\textsuperscript{-1}, or 4.25 $\pm$ 0.05 kcal mol\textsuperscript{-1}. This value differs slightly from the experimental value of -5.01 kcal mol\textsuperscript{-1}~\cite{Kelly2005} and the reported CHARMM22 value of -5.34 $\pm$ 0.12 kcal mol\textsuperscript{-1}~\cite{Anisimov2007}. Deviations are likely due to the fact that the topology used here came from the CGenFF parameter set, rather than the ethanol model compound optimized for the CHARMM protein force field; there are minor differences in partial charge assignments and Lennard-Jones parameters of the constituent atom types.

\subsubsection{Summary and Review of Objectives} \label{dgsolv_summary}

This tutorial guided the user through the process of a complete alchemical transformation of a small molecule in water, including transformation of both Coulombic and Lennard-Jones terms. To review, the learning objectives of this tutorial are:
...
The user was provided relevant context about $\lambda$-spacing in Section~\ref{dgsolv}, including discussion of the linear dependence of electrostatic terms on $\lambda$ and the need for tighter $\lambda$ spacing that is sometimes necessary during transformation of van der Waals terms. The value of $\Delta$G\textsubscript{hydr} was calculated in Section~\ref{dgsolv_run}, using the BAR method employed as in Tutorial 4 (Section~\ref{vdw_ana}).

%%%%%%%%% ADVANCED: PROTEIN-LIGAND BINDING FREE ENERGY %%%%%%%%%

\subsection{Tutorial 6: Protein-Ligand Binding Free Energy} \label{dgbind}

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{t4_lyso_bzf}
\caption{The T4 lysozyme-inhibitor complex.}
\label{t4_bzf_fig}
\end{figure}

This tutorial is available online at \tutorialdgbind.

\subsubsection{Prepare the Ligand Coordinates} \label{dgbind_prepare}

\begin{lstlisting}
$ grep BZF 182L.pdb > bzf.pdb
\end{lstlisting}

(add H in Avogadro) --> bzf\_h.pdb
(rename atoms according to CGenFF - ZFUR)

\begin{lstlisting}
$ grep "ATOM" 182L.pdb | grep -v "BZF" > 182l_protein.pdb
$ cp 182l_protein.pdb complex.pdb
$ echo "TER" >> complex.pdb
$ grep "ATOM" bzf_h.pdb >> complex.pdb
$ echo "END" >> complex.pdb
\end{lstlisting}

\subsubsection{Build the Protein-Ligand Complex} \label{dgbind_complex}

\begin{lstlisting}
$ gmx pdb2gmx -f complex.pdb -o complex_processed.pdb
\end{lstlisting}

\subsubsection{Build Ligand Virtual Sites} \label{dgbind_vsites}

\begin{lstlisting}
$ python3 write_vsites.py -incrd bzf_h.pdb -intop topol.top -initp topol_Other_chain_A2.itp -sele all,O1:C2:C3:C4:C9,C4:C5:C6:C7:C8:C9
\end{lstlisting}

(Copy new LP coordinates into complex\_processed.pdb, move forward with topol\_vsite.top and modified complex\_processed.pdb)

(Insert figure of ZFUR ligand with labeled virtual sites)

\subsubsection{Solvate and Prepare the Complex} \label{dgbind_prep}

\begin{lstlisting}
$ gmx editconf -f complex_processed.pdb -o box.pdb -c -d 1.0 -bt dodecahedron
$ gmx solvate -cp box.pdb -cs spc216.gro -p topol_vsite.top -o solv.pdb
$ gmx grompp -f inputs/ions.mdp -c solv.pdb -p topol_vsite.top -o ions.tpr
$ gmx genion -s ions.tpr -o solv_ions.pdb -p topol_vsite.top -pname NA -nname CL -neutral
\end{lstlisting}

\subsubsection{Set Up Orientation Restraints} \label{dgbind_restraints}

\begin{lstlisting}
$ python3 detect_sites.py
\end{lstlisting}

\begin{figure}[h!]
\centering
\includegraphics[width=\columnwidth]{complex_dist_restraint}
\caption{The distance restraint applied in the complex.}
\label{t4_bzf_dist_fig}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=\columnwidth]{complex_angle_restraints}
\caption{One of the two angle restraints applied in the complex.}
\label{t4_bzf_angle_fig}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=\columnwidth]{complex_dihedral_restraints}
\caption{One of the two dihedral restraints applied in the complex.}
\label{t4_bzf_dihe_fig}
\end{figure}

\subsubsection{Run Complex Free Energy Calculations} \label{dgbind_run}

(job.sh script, basically the same idea as previous tutorials, note the use of restraint keywords)

\textcolor{red}{Free energy of binding in complex, with correction for Boresch-style orientational restraints~\cite{Boresch2024} Note that this is an end-point correction because $\Delta$G is a state function, we only need the correction at the final state, so all k values are their full B-state ($\lambda = 1$) values.}

\begin{equation}
\Delta G_{restraint} = -k_B T ln\left[ \frac{2 \pi V^{\circ}}{r^2_0 sin\theta_A sin\theta_B} \left(\frac {k_r k_{\theta_A} k_{\theta_B} k_{\phi_A} k_{\phi_B}}{(2 \pi k_B T)^5}\right)^{1/2}\right]
\label{eq_boresch_correction}
\end{equation}

\begin{lstlisting}
--- 5-Restraint Correction Results ---
Temperature  : 298.15 K
DG Correction: -30.5631 kJ/mol
DG Correction: -7.3048 kcal/mol
\end{lstlisting}

\subsubsection{Run Solution Free Energy Calculations} \label{dgbind_run_solv}

\textcolor{red}{Same as Tutorial 5, now just for ZFUR, note that vsites are irrelevant since they are non-interacting}

\begin{equation}
\Delta G_{bind} = \Delta G_{complex} - \Delta G_{solv} + \Delta G_{restraint}
\label{eq_dgbind}
\end{equation}

\subsubsection{Summary and Review of Objectives} \label{dgbind_summary}

\textcolor{red}{Restate objectives here and explain where they were met}

%%%%%%%%% ADVANCED: UMBRELLA SAMPLING %%%%%%%%%
\subsection{Tutorial 7: Umbrella Sampling} \label{pmf}

% Re-write: two methanol in water w/CHARMM, inspired by methanol in vacuo system of Hub et al. JCTC 2010
...
It is often of interest to compute a free energy change that is associated with some change in geometry, such as a conformational change in a protein or the binding of two molecules as they approach each other along some path. Attempting to study such processes can be challenging with normal, unbiased MD simulations, as high-energy states (transition states or rare conformations) are not frequently sampled. Therefore, it is difficult to define accurate free energy differences, or the barriers may simply never be overcome on the timescale accessible to conventional MD. To address this challenge, biasing potentials can be applied to enhance sampling of these rare states and thereby more completely define free energy differences.

In this tutorial (available online at \tutorialpmf), the user will be guided through the process of calculating the free energy difference for the separation of two methanol molecules in water. Constructing this system is much like any other, so the initial details of preparing the topology, defining the box, solvation, minimization, and equilibration will only highlight aspects that are unique to subsequent umbrella sampling simulations. This technique restrains the system along one degree of freedom and allows the remaining degrees of freedom to evolve in an unbiased way~\cite{Torrie1977}. To carry out these calculations, a set of configurations is generated along some path within the system (a distance, dihedral angle, etc.), called a "reaction coordinate" (denoted $\xi$), defining the starting points for each of a series of windows that allow sampling in discrete regions of the reaction coordinate. There are many ways to construct initial configurations in each window. In this tutorial, the reaction coordinate is defined as the center-of-mass (COM) connecting two methanol molecules and will employ a non-equilibrium technique called steered molecular dynamics (SMD) to generate the starting configurations for each window.

The principal output of this series of simulations is a potential of mean force (PMF), also denoted $\omega$. A PMF describes how the free energy of a system changes as a function of progress along the restrained degree of freedom at discrete values while all other degrees of freedom are allowed to evolve freely.

\subsubsection{Prepare the Topology} \label{umbrella_topology}

The system of interest will be constructed from coordinates of two methanol molecules separated by a COM distance of 0.25 nm. This system is inspired by a similar \textit{in vacuo} calculation performed by Hub~\textit{et al.} as a test case for the implementation of the Weighted Histogram Analysis Method (WHAM)~\cite{Kumar1992} in GROMACS~\cite{Hub2010}, and previous calculations of this interaction using quantum mechanical methods~\cite{Ghosh2012}. The initial coordinates for these two methanol molecules, in PDB format, are provided in the online tutorial and in the accompanying tutorial files. 

The force field employed in this tutorial is the CHARMM additive force field, with methanol represented by CGenFF. Download the February 2026 release of the CHARMM36 port for GROMACS with CGenFF 5.0 from the MacKerell lab website, as in previous tutorials. Generate the topology as usual with \texttt{pdb2gmx}:

\begin{lstlisting}
$ gmx pdb2gmx -f meoh.pdb -o meoh.gro -water tip3p
\end{lstlisting}
%
Choose CHARMM36 as the force field when prompted.

\subsubsection{Define the Unit Cell and Solvate} \label{umbrella_box}

The box size for umbrella sampling simulations, like in any other system, must be set such that the minimum image convention (see Section~\ref{lyso_solv}) is satisfied at all times, including at the extreme values $\xi$. That is, the length of the biasing potential will be calculated from the minimum periodic distance between the restrained species that define the reaction coordinate. In this example, the reaction coordinate will be defined as the COM distance between two methanol molecules. Given that the reaction coordinate will reach a maximum value of $\sim$3 nm, a box must be defined that maintains this periodic distance, as well. To reflect the intrinsic spherical symmetry of the restrained methanol molecules, the unit cell should be as compact as possible. The most efficient option is a rhombic dodecahedron.

A dodecahedral box that enables a maximum COM distance of 3.0 nm requires unit cell vectors of approximately 6.5 nm to satisfy the minimum image convention when the methanol molecules reach that separation. To define the unit cell for this system, use \texttt{editconf} and solvate with water as in any other system. No ions will be added to this system.

\begin{lstlisting}
$ gmx editconf -f meoh.gro -o newbox.gro -bt dodecahedron -box 6.5 6.5 6.5 -center 0 0 0

$ gmx solvate -cp newbox.gro -cs spc216.gro -o solv.gro -p topol.top
\end{lstlisting}

After doing so, the methanol molecules will be positioned within the unit cell as shown in Figure~\ref{umbrella_box_fig} (after re-wrapping to account for periodicity).

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{umbrella_meoh_box}
\caption{The initial positioning of two methanol molecules in a sufficiently large unit cell to conduct umbrella sampling out to $\xi$ = 3.0 nm.}
\label{umbrella_box_fig}
\end{figure}

\subsubsection{Energy Minimization and Equilibration} \label{umbrella_equil}

Continue preparing the system by performing energy minimization and a short $NPT$ equilibration. Note that it is not strictly necessary to separate equilibration into $NVT$ and $NPT$ phases, as in Tutorial 1 (Section~\ref{lyso_equil}). The necessary \texttt{.mdp} files are provided in the online tutorial. All of the relevant keywords and settings have been discussed previously and will not be elaborated on here.
...
To compute the PMF along a reaction coordinate, a series of simulations is performed. Each of these simulations is independent from the others and is carried out under the influence of a biasing potential that restricts the sampling along some degree of freedom to a region of $\xi$. Commonly, $\xi$ is defined as a distance or dihedral angle. In the present example, $\xi$ is defined as the COM distance between the two methanol molecules. As such, it is necessary to generate a series of configurations at increasing COM distance. There are many approaches to generating configurations, including unbiased simulations from which frames are extracted, manual translation of the target species along a vector that corresponds to $\xi$, or the application of non-equilibrium techniques like SMD to enforce motion along $\xi$, from which frames are extracted. It is this last approach that will be applied in this tutorial. From an SMD trajectory, frames will be extracted corresponding to regular intervals of COM distance, and then individual simulations of these configurations will be performed (Figure~\ref{umbrella_schematic_fig}).
...
To generate a series of configurations that can serve as input into the individual sampling windows, the "pull code" in GROMACS (so named for its ability to apply a biasing potential that "pulls" along a specified vector) will be employed to separate the two methanol molecules in a controlled manner. First, create an index group for each of these molecules:
...
pull_group1_name        = meoh1
pull_group2_name        = meoh2
...
% over a range of {0.2..3.0} by 0.1-nm spacing = 29 windows (0..28)
...
Use the \texttt{md\_pull.mdp} input file to prepare the input for the SMD simulation, and carry it out with \texttt{mdrun}. The simulation is set to run for 270 ps, which should be sufficient to achieve $\sim$3 nm COM separation, starting from the initial 0.25-nm COM distance.
...
\caption{The force on the harmonic spring during SMD separation of the two methanol molecules. The solid line indicates a 50-ps running average of the data points (gray).}
\label{umbrella_smd_pullf_fig}
\end{figure}

After the SMD simulation is finished, plot the contents of \texttt{pull\_pullf.xvg} (Figure~\ref{umbrella_smd_pullf_fig}). This file contains the force on the virtual spring as a function of time. Note that the force on the spring is fairly low during the entire SMD simulation, meaning it's relatively easy to separate the molecules. Forces will evolve differently in different systems. The \texttt{pull\_pullx.xvg} file contains the length of the reaction coordinate as a function of time. Note that the distance steadily increases here, again reflecting that it is fairly easy to separate the two methanol molecules.

\begin{figure}[h!]
\centering
\includegraphics{umbrella_smd_pullx}
\caption{The COM distance between the two methanol molecules during SMD. The solid line indicates a 50-ps running average of the data points (gray).}
\label{umbrella_smd_pullx_fig}
\end{figure}

After completing the SMD simulation, configurations must be extracted from the trajectory to define the starting points for each of the umbrella sampling windows. There are several ways to approach this problem, and here the approach will be to:

\begin{enumerate}
	\item Compute the COM distance between methanol molecules in each snapshot
	\item Specify the desired values of $\xi$ and window intervals as input into umbrella sampling
	\item Extract the frame with the closest COM distance for each desired value of $\xi$
\end{enumerate}

Compute the time series of the COM distances with the \texttt{distance} module. Note the use of single quotes (') to enclose the entire selection string and double quotes (") to enclose group names referenced from \texttt{index.ndx}.

\begin{lstlisting}
$ gmx distance -s pull.tpr -f pull.xtc -select 'com of group "meoh1" plus com of group "meoh2"' -n index.ndx -oall dist.xvg
$ python3 find_windows.py
\end{lstlisting}

The \texttt{find\_windows.py} script attempts to find COM distances in the range of 0.2 $-$ 3.0 nm, with an interval of 0.1 nm. It subsequently calls the GROMACS \texttt{trjconv} module to write out a coordinate file at the time corresponding to the closest COM distance to the desired value. In this example, 28 configurations will be produced, the first named \texttt{window0.gro} and the last \texttt{window27.gro}.
...
Prepare the input \texttt{.tpr} files for each $NPT$ equilibration and carry out those simulations. The example shown is for the first window, but needs to be repeated for all 28 windows, which is easily accomplished with a for-loop in a shell script.

\begin{lstlisting}
$ gmx grompp -f npt_umbrella.mdp -c window0.gro -r window0.gro -p topol.top -n index.ndx -o npt0.tpr
$ gmx mdrun -deffnm npt0
\end{lstlisting}

After equilibration, production umbrella sampling MD simulations are performed. The template \texttt{md\_umbrella.mdp} file provided in the online tutorial and in the accompanying tutorial files modifies the \texttt{md\_pull.mdp} pull settings slightly. In this case, \texttt{pull\_coord1\_start} is set to "no" and the target COM separation distance is specified directly using the \texttt{pull\_coord1\_init} keyword. The \texttt{write\_mdp.pl} Perl script takes the template file and produces a series of suitable \texttt{.mdp} files, which again can be carried out in a for-loop:

\begin{lstlisting}
$ gmx grompp -f md_umbrella_0.mdp -c npt0.gro -t npt0.cpt -p topol.top -r npt0.gro -n index.ndx -o umbrella0.tpr
...
The above command computes a PMF in the units of kJ mol\textsuperscript{-1} and writes it to a file called \texttt{profile.xvg}. The corresponding umbrella histograms, $h(\xi)$, are also produced (\texttt{histo.xvg}). The PMF that results is not actually the final answer, however. Given that we are applying restraints to a system with spherical symmetry, we must correct for the restriction in spherical volume accessible to the methanol molecules. Therefore, we must apply a volume entropy correction equal to $2 k_B T ln(4)$, where $r$ is the restraint length:

\begin{equation}
\omega = \omega_{WHAM} + 2 k_B T ln(r)
\label{eq_pmf_correction}
\end{equation}

The online tutorial and accompanying tutorial files provide a Python script called \texttt{correct\_pmf.py} handles this volume entropy correction by reading in the \texttt{profile.xvg} file produced by the \texttt{wham} program. It is also worth mentioning how WHAM computes the actual PMF values along $\xi$. By convention, the leftmost window ({\em i.e.} lowest value of $\xi$) is assigned a free energy value of zero. The energies in subsequent windows are all defined relative to this value. It may be more useful to shift some physically relevant or otherwise convenient point to zero. Such a practice does not change the result at all, it is a systematic translation of the PMF curve and the final value of $\Delta$G is the same. The GROMACS \texttt{wham} program has the ability to shift the final PMF curve to place the zero point at a user-defined location, but in this case, the offset is also handled by the \texttt{correct\_pmf.py} script, by subtracting the value printed to the terminal.

\begin{lstlisting}
$ python correct_pmf.py
...
Shifted bulk baseline energy by subtracting: -21.1317 kJ/mol
\end{lstlisting}

Next, one should assess the quality of the sampling obtained during the umbrella simulations. This information is plotted in the \texttt{histo.xvg} file, which contains histograms of the $\xi$ values sampled in each of the windows. Note that \texttt{histo.xvg} is contains multiple data series and should be plotted in XmGrace as such:

\begin{lstlisting}
$ xmgrace -nxy histo.xvg
\end{lstlisting}

The final PMF and the corresponding histograms are shown in Figure~\ref{umbrella_composite_pmf_histo_fig}. The PMF features two minima, the global minimum around $\xi = 0.44$ nm and another local minimum around $\xi = 0.75$ nm), separated by a barrier around $\xi = 0.6$ nm. The study by Ghosh \textit{et al.} identified contact pairs at C$-$C distances ($r_{CC}$) of 0.37 and 0.44 nm and a solvent-separated pair at $r_{CC}$ of 0.7 nm, with a barrier around $r_{CC} = 0.54$ nm~\cite{Ghosh2012}. In light of the difference between reaction coordinates (COM distance here vs. $r_{CC}$ in the previous study) and the use of an empirical force field vs. a fully quantum mechanical model, the agreement is quite good.

\begin{figure}[h!]
\centering
\includegraphics[width=\columnwidth]{umbrella_composite_pmf_histo}
...
In this tutorial, the user has been guided through the process of performing an SMD simulation to separate two methanol molecules in a box of water. From the SMD trajectory, defining a three-dimensional reaction coordinate, snapshots were extracted at roughly regular intervals. These snapshots were subjected to additional equilibration and extended simulations to compute a PMF profile, which (after correction) yields an estimate of $\Delta$G for dissociation of these two molecules. To review, the objectives for this tutorial were:
...
%%%%%%%%% ADVANCED: BIPHASIC %%%%%%%%%
\subsection{Tutorial 8: Biphasic Systems} \label{biphasic}
...
%%%%%%%%% ADVANCED: VIRTUAL SITES %%%%%%%%%
\subsection{Tutorial 9: Virtual Sites} \label{vsite}
